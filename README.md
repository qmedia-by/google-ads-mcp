...
> и Реестр Аккаунтов — инструменты `registry_*`, читающие связку
> «Клиент → Аккаунты» из закрытой таблицы агентства. Он же служит allowlist'ом
> для Google Ads: Аккаунт Google Ads вне Реестра сервер не отдаёт. Для
> остальных Провайдеров Реестр — навигация: их кабинеты живут на чужих
> серверах, и запретить там что-либо он не может.
