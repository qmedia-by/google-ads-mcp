...
| `ads_mcp/access_control.py` | **новый** — allowlist Аккаунтов по `GOOGLE_ADS_ALLOWED_CUSTOMER_IDS` |
...
| `GOOGLE_ADS_ALLOWED_CUSTOMER_IDS` | allowlist Аккаунтов, `customer_id` через запятую. Пусто = без ограничений; строка обязана присутствовать — см. «Контроль доступа» |
...
| **`GOOGLE_ADS_ALLOWED_CUSTOMER_IDS`** | `.env` на сервере | **какие именно Аккаунты** разрешено запрашивать |
...
Первых двух списков мало. Менеджер, заведённый в MCC, дотягивается до **любого** Аккаунта агентства, а не только до Аккаунтов своих Клиентов, — достаточно опечатки в `customer_id`, чтобы агент вытащил цифры чужого Клиента. Это добавление форка: в upstream такого слоя нет, и ранние наши документы описывали переменную как существующую по ошибке. См. `qmedia-ads-agent/docs/adr/0003`.

`GOOGLE_ADS_ALLOWED_CUSTOMER_IDS` — список `customer_id` через запятую. Значение печатает генератор из Реестра, который тем самым перестаёт быть холостым:

```bash
node scripts/allowed-customer-ids.mjs     # в qmedia-ads-agent
```
...
`metadata_get_resource_metadata` не трогаем: он описывает схему API, а не данные Аккаунта, и `customer_id` не принимает.

Выдача фильтруется, а не отклоняется, потому что это инструмент обнаружения: Аккаунт, который агенту всё равно запретят запросить, незачем предлагать ему как вариант. Обратная сторона — при пустом пересечении Менеджер увидит «нет доступных Аккаунтов» вместо внятной ошибки, неотличимо от незаведённости в MCC выше; в логах сервера при этом будет строка `GOOGLE_ADS_ALLOWED_CUSTOMER_IDS hid N of M accessible accounts`.

Проверка идёт **до** обращения к Google: отказ не тратит квоту и не требует валидных учётных данных.

Правила разбора значения:

- **Пусто или не задано — ограничений нет.** Это поведение upstream, и оно же позволяет гонять тот же образ локально без переменной.
...
- Запись, не сводящаяся к цифрам, отбрасывается с `WARNING` в логе.
- Если переменная задана, но **ни одной валидной записи** не осталось, запрещены все Аккаунты. Это намеренный fail-closed: allowlist, который молча ничего не ограничивает, — ровно та дыра, ради которой всё это писалось. Отказ в этом случае прямо говорит, что сломан сервер, а не запрос.

Preflight деплоя проверяет **наличие строки**, а не значения: выключить allowlist можно, забыть его — нет.

Из-за этого первый деплой после появления переменной упадёт на preflight: в `.env` на уже работающем сервере строки нет. Это разовая ручная правка, и её лучше сделать заранее, до пуша в `dev`:

```bash
cd $DEPLOYMENT_FOLDER
echo 'GOOGLE_ADS_ALLOWED_CUSTOMER_IDS=<из генератора, через запятую>' >> .env
```
...
5. **`.env`.** Скопировать `docker/.env.server.example` в `DEPLOYMENT_FOLDER/.env` и заполнить; значение `GOOGLE_ADS_ALLOWED_CUSTOMER_IDS` даёт `node scripts/allowed-customer-ids.mjs` в `qmedia-ads-agent`. Проверка: `grep CHANGE_ME .env` ничего не находит.
...
8. **Менеджер.** Его Google-аккаунт — в Test users (GCP) **и** пользователем в MCC (Google Ads). Две операции, см. «Контроль доступа». Проверка: после входа `customers_list_accessible_customers` возвращает непустой список, и в нём виден id MCC.
